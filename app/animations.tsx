...
const ShoulderLateralCable = ({ color }: { color: string }) => (
  // FRONTALE IN PIEDI CAVI INCROCIATI
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
    <line x1="10" y1="10" x2="10" y2="90" stroke="#444" strokeWidth="6"/>
    <line x1="90" y1="10" x2="90" y2="90" stroke="#444" strokeWidth="6"/>
...
const ShoulderLateralMachine = ({ color }: { color: string }) => (
  // FRONTALE SEDUTO MACCHINA ALZATE
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
...
const TricepOverhead = ({ color }: { color: string }) => (
  // PROFILO CAVO BASSO ESTENSIONI DIETRO NUCA
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
...
const BackLatTriangle = ({ color }: { color: string }) => (
  // FRONTALE LAT MACHINE CON TRIANGOLO STRETTO
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
...
const BackLatPulldown = ({ color }: { color: string }) => (
  ...
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
    ...
    <line x1="42" y1="43" x2="58" y2="43" stroke={color} strokeWidth="5" strokeLinecap="round"/>
    <line x1="50" y1="43" x2="50" y2="70" stroke={color} strokeWidth="5" strokeLinecap="round"/>
    <path d="M 45 70 L 45 95" stroke={color} strokeWidth="4.5" fill="none"/><path d="M 55 70 L 55 95" stroke={color} strokeWidth="4.5" fill="none"/>
...
const BackPullup = ({ color }: { color: string }) => (
  ...
  <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
    ...
    <g><animate attributeName="opacity" values="1;0;1" keyTimes="0;0.5;1" dur="1.8s" repeatCount="indefinite"/><circle cx="50" cy="35" r="5" stroke={color} strokeWidth="2.5" fill="none"/><line x1="42" y1="43" x2="58" y2="43" stroke={color} strokeWidth="5" strokeLinecap="round"/><line x1="50" y1="43" x2="50" y2="70" stroke={color} strokeWidth="5" strokeLinecap="round"/><path d="M 45 70 L 45 95 M 55 70 L 55 95" stroke={color} strokeWidth="4.5" fill="none" strokeLinejoin="round"/><path d="M 30 15 L 36 30 L 42 43" stroke={color} strokeWidth="4" fill="none" strokeLinejoin="round"/><path d="M 70 15 L 64 30 L 58 43" stroke={color} strokeWidth="4" fill="none" strokeLinejoin="round"/></g>
    <g opacity="0"><animate attributeName="opacity" values="0;1;0" keyTimes="0;0.5;1" dur="1.8s" repeatCount="indefinite"/><circle cx="50" cy="15" r="5" stroke={color} strokeWidth="2.5" fill="none"/><line x1="42" y1="23" x2="58" y2="23" stroke={color} strokeWidth="5" strokeLinecap="round"/><line x1="50" y1="23" x2="50" y2="50" stroke={color} strokeWidth="5" strokeLinecap="round"/><path d="M 45 50 L 45 75 M 55 50 L 55 75" stroke={color} strokeWidth="4.5" fill="none" strokeLinejoin="round"/><path d="M 30 15 L 25 35 L 42 23" stroke={color} strokeWidth="4" fill="none" strokeLinejoin="round"/><path d="M 70 15 L 75 35 L 58 23" stroke={color} strokeWidth="4" fill="none" strokeLinejoin="round"/></g>
  </svg>
);
...
// ==========================================
// IL CERVELLO DI MAPPATURA GLOBALE
// ==========================================
    ...
    <div className="w-16 h-16 bg-neutral-900 rounded-lg border border-neutral-700 flex items-center justify-center shadow-inner shrink-0 p-1">
      {(() => {
        switch (animKey) {
          ...
          case 'chest_flye_db': return <ChestFlyeDb color={color} />; // Top-Down
          case 'chest_cable_flat': return <ChestCableFlat color={color} />; // Top-Down Panca
          case 'chest_cable_seated': return <ChestCableSeated color={color} />; // Frontale Seduto

          // --- SPALLE E TRICIPITI ---
          case 'shoulder_barbell_standing': return <ShoulderMilitary color={color} />; // Standing
          case 'shoulder_db_seated': return <ShoulderDbSeated color={color} />; 
          case 'shoulder_machine': return <ShoulderDbSeated color={color} />; // Fallback a DB seduto
          case 'shoulder_lateral_db': return <LateralDb color={color} />;
          case 'shoulder_lateral_cable': return <ShoulderLateralCable color={color} />; // Incrociati Frontale
          case 'shoulder_lateral_machine': return <ShoulderLateralMachine color={color} />; // Seduto Frontale
          
          case 'tricep_close_grip': return <TricepCloseGrip color={color} />; // Gomiti incollati
          case 'tricep_pushdown': return <TricepPushdown color={color} />; 
          case 'tricep_french_press': return <TricepFrenchPress color={color} />; 
          case 'tricep_overhead_cable': return <TricepOverhead color={color} />; // Dal basso dietro
          case 'tricep_dips': return <ChestBarbellFlat color={color} />; // Metti temporaneamente una panca se manca Dips

          // --- TIRATA / SCHIENA E BICIPITI ---
          case 'back_pullup': return <BackPullup color={color} />;
          case 'back_lat_pulldown': return <BackLatPulldown color={color} />; // Sbarra avanti
          case 'back_lat_triangle': return <BackLatTriangle color={color} />; // Triangolo
          case 'back_row_barbell': return <BackRowBarbell color={color} />;
          case 'back_row_db': return <BackRowBarbell color={color} />; 
          case 'back_pulley': return <BackPulley color={color} />; // Pulley basso
